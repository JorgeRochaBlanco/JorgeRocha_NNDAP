...
BASE_PATH_DATA = "C:/Jorge/NNDAP/data/"
BASE_PATH_STORAGE = "C:/Jorge/NNDAP/farming_storage/"
...
COWS_PARQUET = 'cows.parquet'
SENSORS_PARQUET = 'sensors.parquet'
MEASUSER_PARQUET = 'measurements.parquet'
...
def bulk_load_cows(df_cows):
    ...
    try:
        os.remove(BASE_PATH_STORAGE + COW_MASTER_FILE)
    except OSError:
        pass
...
def bulk_load_sensors(df_sensors):
    ...
    #File deletion (storage)
    try:
        os.remove(BASE_PATH_STORAGE + SENSORS_MASTER_FILE)
    except OSError:
        pass
...
def bulk_load_measures(df_measures):
    ...
    #File deletion (storage)
    try:
        os.remove(BASE_PATH_STORAGE + MEASURES_MASTER_FILE)
    except OSError:
        pass
    ...
    for index, row in df_measures.iterrows():
        ...
        try:
            ...
            print("Error in REST API for inserting measure: " + str(payload_measures))
    return ok_inserts, ko_inserts
